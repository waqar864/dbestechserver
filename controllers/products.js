...
exports.getProducts = async function (req, res) {
    try {
       ...
       if(req.query.criteria){
            ...
            products = await Product.find(query).select('-images -reviews -size').skip((page - 1) * pageSize).limit(pageSize);
        } else if (req.query.category) {
            products = await Product.find({category: req.query.category}).select('-images -reviews -size').skip((page - 1) * pageSize).limit(pageSize);
       }else {
            products = await Product.find().select('-images -reviews -size').skip((page - 1) * pageSize).limit(pageSize);
       }
       if(!products) {
